...
function binding($user,$mail){
    $mail = str_replace(' ','',$mail);
    $mysql = new SaeMysql();
    
    if($mail == "@"){
         $mysql->runSql("DELETE FROM mail WHERE user='$user'");
         return '取消邮件提醒成功。';
    }else{
    	if($mysql->getData("SELECT user FROM mail WHERE user='$user'")){
			$mysql->runSql("UPDATE mail SET mail = '$mail' WHERE user='$user'");
       		return '绑定邮箱已更新。';
		}else{
			$mysql->runSql("INSERT INTO mail (user,mail) VALUES ('$user','$mail')");
        	return '绑定邮箱成功。';
    	}
    }
}
...
function auto($user,$kuaid_num){
	...
	if($kuaidi_update['status'] !== '200'){
        return "自动识别单号失败。\n请输入\"快递名称 单号\"查询，如：中通 762145853981";
	}else{
        $update_time = $kuaidi_update['data']['0']['time'];
        
        if(validate($user)){
    		if(!$mysql->getData("SELECT num FROM task WHERE num='$kuaid_num' and  user='$user'")){
        		$mysql->runSql("INSERT INTO task (user,num,kuaidi,last) VALUES ('$user','$kuaid_num','$kuaidi_type','$update_time')");
        	}
        }
        
        $data = '单号：' .$kuaid_num. "\n最新状态：\n" . $update_time .' '. $kuaidi_update['data']['0']['context'] ."\n<a href=\"http://kdtx.sinaapp.com/detail.php?type=$kuaidi_type&postid=$kuaid_num\">点击查询详情</a>";
    	return $data;
	}
}

function qx($user,$kuaid_num){
    $mysql = new SaeMysql();

    $kuaid_num = substr($kuaid_num,2);
    if($mysql->getData("SELECT num FROM task WHERE num='$kuaid_num' and  user='$user'")){
        $mysql->runSql("DELETE FROM task  WHERE num = '$kuaid_num'");
        return '成功取消此单快递提醒！';
    }else{
    	return '取消此单快递提醒失败，请检查单号是否正确。';
    }
}
...
function query($user,$text){
    ...
    $info = explode(" ",$text);
    $kuaid_num = $info[1];
    $kuaidi_type = Pinyin($info[0]);
    
	$kuaidi_update = json_decode(file_get_contents('http://baidu.kuaidi100.com/query?type=' .$kuaidi_type. '&postid=' .$kuaid_num),1);
            
	if($kuaidi_update['status'] !== '200'){
    	return array('Content' => "查询失败,请检查您的输入是否正确。\n您的此次查询已记录，我们会尽快排查问题的。",'flag' => '1');
	}else{
        $update_time = $kuaidi_update['data']['0']['time'];
